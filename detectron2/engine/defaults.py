...
class DefaultPredictor:
    ...
    def __call__(self, original_image_path, c_cfg):
        """
        Args:
            original_image_path paths to images
        ...
        original_images=[]
        import numpy as np
        if not isinstance(original_image_path,list):
            original_image_path=[original_image_path]

        for path in original_image_path:
            if isinstance(path, str):
                ...
                ## if the passed params is numpy array
                img = path
            else:
                raise NotImplementedError('Please pass numpy array or string image path to the Predictor..')

            original_img = img.copy()   # user가 볼 image
            img = img_Contrast(img)     # image preprocessing

            h, w = img.shape[:2]
            if self.input_format == "RGB":  # # whether the model expects BGR inputs or RGB
                img = img[:, :, ::-1]
            original_images.append(original_img)
            ### resize image to min,max size
            resized_img = self.aug.get_transform(img).apply_image(img)
            ## convert to pytorch tensor, transpose channel dim up
            image = torch.as_tensor(resized_img.astype("float32").transpose(2, 0, 1))
            ## preparing model input dictionary
            input = {"image": image, "height": h, "width": w}
            inputs.append(input)
...
class DefaultTrainer(SimpleTrainer):
    ...
    def __init__(self, cfg):
        ...
        Args:
            cfg (CfgNode):
        """
        logger = logging.getLogger("detectron2")            # logging객체 생성
        if not logger.isEnabledFor(logging.INFO):           # 프로그램이 정상작동중인지 확인
            setup_logger()      # setup_logger is not called for d2
        cfg = DefaultTrainer.auto_scale_workers(cfg, comm.get_world_size())     # comm.get_world_size() = 1 기대(초기 학습시)
    ...
    def build_model(cls, cfg):
        ...
        logger = logging.getLogger(__name__)        # detectron2.engine.defaults
