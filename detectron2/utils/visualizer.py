...
class Visualizer:
    ...
    def draw_instance_predictions(self, predictions):
        ...
        if boxes is not None : 
            masks, boxes, labels, _,  = self.overlay_instances(
            masks=masks,
            boxes=boxes,
            labels=labels,
            keypoints=keypoints,
            ...
            return masks, boxes, labels, self.output
        else : 
            return None, None, None, self.output
    ...
    def draw_panoptic_seg_predictions(
        ...
        try:
            ...
            colors = None
        _, _, _, _ = self.overlay_instances(masks=masks, labels=labels, assigned_colors=colors, alpha=alpha)
    ...
    def draw_dataset_dict(self, dic):
        ...
        if annos:
            ...
            _, _, _, _ = self.overlay_instances(
    ...
    def overlay_instances(
        ...
        *,
        ...
        for i in range(num_instances):
            # score = int(labels[i].split(" ")[-1][:-1])
            # label = labels[i].split(" ")[0]
            # if score < 95  or score == 100:
            #     score = random.randrange(95, 101)
            #     labels[i] = f"{label} {score}%"
            # # print(label)
            # print(score)
