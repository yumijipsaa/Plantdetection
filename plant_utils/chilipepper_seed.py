...
class Chilipepper_seed:
    def __init__(self, config, class_name_list):
        ...
        self.config = config
        self.plant_type = 'chilipepper_seed'
    ...
    def set_outer_inner_idx(self):
        ...
        for outer_objects in outer_objects_chilipepper_seed:  # 외부 object를 for문으로 나열
            ...
            for outer_info in self.bbox[outer_objects]:  # 외부 object의 index를 나열
                ...
                for inner_objects in inner_objects_chilipepper_seed:  # 내부 object를 나열
    ...
    def set_dict(self):
        ...
        # index를 저장할 dict
        for object_name in plant_object_idx_chilipepper_seed:
            self.idx_dict[object_name] = []

            # self.set_outer_inner_idx 에서 사용됨
        for object_name in inner_objects_chilipepper_seed:
            self.center_point[object_name] = []
        for object_name in outer_objects_chilipepper_seed:
            self.bbox[object_name] = []

        # object의 개수를 저장할 dict
        for obj_name in count_object_chilipepper_seed:
        ...
        for idx, output in enumerate(outputs["instances"].pred_classes):
            ...
            for object_name in plant_object_idx_chilipepper_seed:
                if self.class_name_list[output] == object_name:
                    ...
                    if object_name in inner_objects_chilipepper_seed:
                    ...
                    elif object_name in outer_objects_chilipepper_seed:
                    ...
                    if object_name in count_object_chilipepper_seed:
    ...
    def set_segmentations_dict(self, idx):
        for object_name in result_objects_chilipepper_seed:
    ...
    def calculate_coordinates(self, outputs, img):
        ...
        # object counting
        for object_name in count_object_chilipepper_seed:
            bbox_list = list()
    ...
    def get_cap_fruit_points_(self, outer_idx, inner_idx, fruit_point_dict,
        ...
        if not center_flag:  # cap이 fruit의 center box에 포함되지 않은 경우(가장자리에 위치한 경우)
            # height계산
            fruit_height = self.compute_fruit_height_chilipepper_seed(outer_idx, inner_idx, is_cap, is_cap_2)
    ...
    def compute_fruit_height_chilipepper_seed(self, outer_idx, inner_idx, is_cap, is_cap_2):
        # sub_function
