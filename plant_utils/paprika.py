...
class Paprika:
    ...
    def set_outer_inner_idx(self):
        ...
        for outer_objects in outer_objects_paprika:  # 외부 object를 for문으로 나열
            ...
            for outer_info in self.bbox[outer_objects]:  # 외부 object의 index를 나열
                ...
                for inner_objects in inner_objects_paprika:  # 내부 object를 나열
    ...
    def set_dict(self):
        ...
        # index를 저장할 dict
        for object_name in plant_object_idx_paprika:
            self.idx_dict[object_name] = []

            # self.set_outer_inner_idx 에서 사용됨
        for object_name in inner_objects_paprika:
            self.center_point[object_name] = []
        for object_name in outer_objects_paprika:
            self.bbox[object_name] = []

        # object의 개수를 저장할 dict
        for obj_name in count_object_paprika:
        ...
        for idx, output in enumerate(outputs["instances"].pred_classes):
            ...
            for object_name in plant_object_idx_paprika:
                if self.class_name_list[output] == object_name:
                    ...
                    if object_name in inner_objects_paprika:
                    ...
                    elif object_name in outer_objects_paprika:
                    ...
                    if object_name in count_object_paprika:
    ...
    def set_segmentations_dict(self, idx):
        for object_name in result_objects_paprika:
    ...
    def calculate_coordinates(self, outputs, img):
        ...
        # object counting
        for object_name in count_object_paprika:
            bbox_list = list()
        ...
        print(f"Usable part list:")
        for k,v in coordinates_dict.items():
            print(f'{k}: {len(coordinates_dict[k])}')
    ...
    def get_draw_leaf_info(self):
        ...
        for outer_idx, inner_info_list in self.outer_inner_idx['leaf'].items():
            print(f'{outer_idx} -- {len(inner_info_list)}')
            outer_idx = int(outer_idx)
            ...
            if len(inner_info_list) <= 0:
                ...
                if check_availability:
                    ...
                    print(f'check_availability is false ...')
    ...
    def find_width_point_midrid(self, leaf_coordinates, bbox_leaf, width_or_height):
        ...
        max_length = 0
        max_length_idx = None
        # print(f'{len(edge_point_list)} -- {edge_point_list}')
