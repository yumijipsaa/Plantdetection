...
class Strawberry:
    ...
    def set_outer_inner_idx(self):
        ...
        for outer_objects in outer_objects_strawberry:  # 외부 object를 for문으로 나열
            ...
            for outer_info in self.bbox[outer_objects]:  # 외부 object의 index를 나열
                ...
                for inner_objects in inner_objects_strawberry:  # 내부 object를 나열
    ...
    def set_dict(self):
        ...
        # index를 저장할 dict
        for object_name in plant_object_idx_strawberry:
            self.idx_dict[object_name] = []

            # self.set_outer_inner_idx 에서 사용됨
        for object_name in inner_objects_strawberry:
            self.center_point[object_name] = []
        for object_name in outer_objects_strawberry:
            self.bbox[object_name] = []

        # object의 개수를 저장할 dict
        for obj_name in count_object_strawberry:
        ...
        for idx, output in enumerate(outputs["instances"].pred_classes):
            ...
            for object_name in plant_object_idx_strawberry:
                if self.class_name_list[output] == object_name:
                    ...
                    if object_name in inner_objects_strawberry:
                    ...
                    elif object_name in outer_objects_strawberry:
                    ...
                    if object_name in count_object_strawberry:
    ...
    def set_segmentations_dict(self, idx):
        for object_name in result_objects_strawberry:
    ...
    def calculate_coordinates(self, outputs, img):
        ...
        # object counting
        for object_name in count_object_strawberry:
            bbox_list = list()
        ...
        for outer_idx, inner_info_list in self.outer_inner_idx['leaf'].items():
            ...
            if len(inner_info_list) <= 0:
                ...
                if len(self.point_dict["leaf_width_edge_coordinates"]) == 0:
                    point_dict_list.append(self.point_dict)
                    continue
                self.point_dict["leaf_width_edge_coordinates"] = get_width_point(self.point_dict["leaf_width_edge_coordinates"], 30)
    ...
    def find_width_point_midrid(self, leaf_coordinates, bbox_leaf, width_or_height):
        ...
        if width_or_height == "height":
            ...
            x_min_center_box, x_max_center_box, y_min_center_box, y_max_center_box = int(x_min + width / 10), int(
                x_max - width / 4), y_min, y_max
        ...
        # width만 구할 수 있다.
            ...
            for length_difference, coordinates_two_point in length_difference_list:
                if lowst_lenth > length_difference:
                    ...
                    # width의 양 끝점 좌표
            width_points_list = get_width_point(width_fruit_coordinates, 30)
    ...
    def compute_fruit_height_strawberry(self, outer_idx, inner_idx):
        ...
        fruit_points_top = tmp_fruit_points[
                           :int(len(tmp_fruit_points) / 2)]  # 가까운 point를 찾을땐 fruit의 상단 boundary points만 사용
        fruit_points_bottom = tmp_fruit_points[
                              int(len(tmp_fruit_points) / 2):]  # 먼 point를 찾을땐 fruit의 하단 boundary points만 사용
    ...
    def get_leaf_width_height(self, bbox_leaf, leaf_coordinate):
        ...
        if bbox_height >= bbox_width:
            ...
            tmp_points_for_width = [x for x in leaf_coordinate if x[0] <= x_min + bbox_width/5]
        ...
        # 가로 길이가 더 긴 경우 가로 길이를 기준으로 1/5지점 분할 후 탐색
            ...
            tmp_points_for_width = [x for x in leaf_coordinate if x[1] <= y_min + bbox_height/5]
