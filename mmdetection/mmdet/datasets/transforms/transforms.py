...
@TRANSFORMS.register_module()
class RotateTransform(GeomTransform):
    """이미지, 바운딩 박스, 마스크, 세그멘테이션 맵을 회전하는 변환 클래스.

    필수 입력:
    - img
    - gt_bboxes (BaseBoxes[torch.float32], 선택 사항)
    - gt_masks (BitmapMasks | PolygonMasks, 선택 사항)
    - gt_seg_map (np.uint8, 선택 사항)

    Args:
        prob (float): 변환 적용 확률 (0~1). 기본값: 1.0
        level (int, optional): 변환 강도. 기본값: None
        max_angle (float): 회전할 최대 각도(± 범위). 기본값: 30도
        interpolation (str): 이미지 보간(interpolation) 방법. 기본값: 'bilinear'
    """
    ...
    def __init__(self,
        ...
        assert 0. <= max_angle <= 180., \
            f'회전 각도 `max_angle` 값은 0~180 사이여야 합니다. 현재: {max_angle}'
        super().__init__(prob=prob, level=level)
        self.max_angle = max_angle
        self.interpolation = interpolation

    def _get_homography_matrix(self, results: dict, angle: float) -> np.ndarray:
        """회전 변환을 위한 2D 변환 행렬을 반환."""
        h, w = results['img_shape'][:2]
        center = (w / 2, h / 2)  # 회전 중심: 이미지 중앙
        rotation_matrix = cv2.getRotationMatrix2D(center, angle, 1.0)
        homography_matrix = np.vstack([rotation_matrix, [0, 0, 1]])  # 3x3 변환 행렬
        return homography_matrix

    def _transform_img(self, results: dict, angle: float) -> None:
        """이미지를 회전 변환."""
        h, w = results['img_shape'][:2]
        center = (w / 2, h / 2)
    ...
    def _transform_masks(self, results: dict, angle: float) -> None:
        """마스크 회전 변환."""
        h, w = results['img_shape'][:2]
        results['gt_masks'] = results['gt_masks'].rotate(
            angle, center=(w / 2, h / 2)
        )

    def _transform_seg(self, results: dict, angle: float) -> None:
        """세그멘테이션 맵 회전 변환."""
        h, w = results['img_shape'][:2]
        center = (w / 2, h / 2)
